 ...
 * Pixline - http://pixline.net/ */
	function get_parameters($text){
		if(eregi('id[ ]?=[ ]?[0-9]{1,}', $text, $matches)){//Id
			$par[]=$matches[0];
		}
		if (eregi('name=[a-z]+', $text, $matches)){//Category Name
				$par[]=$matches[0];
		}
		if(eregi('orderby=[a-z]+', $text, $matches)){//OrderBy
				$par[]=$matches[0];
		}
		return $par;
	}

	function build_getposts($par){
		$sql='get_posts(\'';
		foreach($par as $ag){
			$sql.=$ag."&amp;";
		}
...
function list_category_posts($content){
	...
	if ( stristr( $content, '[catlist' )) {
		...
		if(eregi('[catlist [a-z]+=[a-z0-9]', $content, $matches)){
			foreach($matches as $linea){
				$linea=get_parameters($linea);
				$linea=build_getposts($linea);
				$output = "<ul class='lcp_catlist'>";
				$catposts = $linea;
				foreach($catposts as $single):
					$output .= "<li><a href='".get_permalink($single->ID)."'>".$single->post_title."</a></li>";
